...
print(f"TRAVELPAYOUTS: {'已配置' if os.environ.get('TRAVELPAYOUTS_TOKEN') else '未设置'}")
...
try:
    ...
    s = TravelpayoutsSource()
    result = s.fetch("PVG", "MCO", "2026-06-20")
    flights = result.get("flights", [])
    print(f"[Travelpayouts] 成功，返回 {len(flights)} 个航班")
except Exception as e:
    print(f"[Travelpayouts] 失败：{e}")
