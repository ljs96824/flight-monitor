...
class LegacyNotificationRendererUnavailable(RuntimeError):
    """The legacy HTML renderer was retired after its semantics became incomplete."""
...
def format_html_message(
    ...
    """Reject the retired renderer and point callers to the current payload pipeline."""
    raise LegacyNotificationRendererUnavailable(
        "旧HTML通知入口已退役;请先调用build_notification_payload,"
        "再按渠道使用render_email、render_detail_html或render_pushplus_sections"
    )
