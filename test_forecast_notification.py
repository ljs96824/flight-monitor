...
class ForecastNotificationTest(unittest.TestCase):
    def test_market_envelope_has_no_constraint_fingerprint(self):
        ...
        for depart_index in range(5):
            ...
            for offset in range(12):
                cells.append(
                    {
                        "depart_date": depart,
                        "observed_day": f"2026-09-{10 + offset:02d}",
                        "days_to_departure": 21 + depart_index - offset,
                        "min_price": 100 + offset,
                        "degraded": False,
                        "min_sources": ["juhe"],
                        "lineage_complete": True,
                        "round_ids": [f"round-{depart_index}-{offset}"],
                    }
                )
        cells.append(
            {
                "depart_date": "2026-10-01",
                "observed_day": "2026-09-30",
                "days_to_departure": 1,
                "min_price": 999,
                "degraded": False,
                "min_sources": ["juhe"],
                "lineage_complete": True,
                "round_ids": ["future-round"],
            }
        )
        shape = {
            t_value: {
        ...
        with patch(
            ...
            "forecast.build_shapes_by_regime", return_value={"normal": shape}
        ), patch(
            "forecast.estimate_level",
            return_value={"reliable": True, "n": 5, "value": 100},
        ), patch("forecast.walk_forward_backtest") as mocked, patch(
        ...
        backtest_cells = mocked.call_args.args[0]
        self.assertTrue(backtest_cells)
        self.assertTrue(
            all(item["observed_day"] <= "2026-09-21" for item in backtest_cells)
        )
        self.assertIn(
    ...
    def test_email_section_requires_eligible_payload(self):
        ...
        payload = {
            "forecast": {
                ...
                "current_market_reference": {"median": 100},
            },
            "price_tiers": {"unit_roundtrip": 300},
        }
