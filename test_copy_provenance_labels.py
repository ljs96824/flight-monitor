...
class PriceReferenceTierTextTest(unittest.TestCase):
    def test_price_signal_names_the_actual_recent_sample_count(self):
        signal = analyzer.build_price_signal(
            900,
            target_price=800,
            price_history=[1000, 1050, 1100],
        )

        self.assertEqual(
            signal["summary"],
            "搜索参考价处于近期低位（近3次同条件采集）",
        )

    def test_trigger_reasons_bind_history_and_user_setting_tiers(self):
        result = analyzer.determine_push_type(
            1200,
            target_price=1000,
            max_budget=1500,
            price_history=[700, 800, 900],
            analysis_result={},
        )
