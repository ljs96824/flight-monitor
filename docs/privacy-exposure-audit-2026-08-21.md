...
### 5.3 历史快照产物曝露评估（2026-08-24）

本次只读扫描 `before.json` 与 `after.json` 的全部 Git 对象：覆盖 2026-06-02 至 2026-08-23 的 10 个提交、20 个可解析 JSON blob，共得到 64 条订阅叶子字段路径。扫描仅汇总字段名和计数，未在报告中复制历史值。

曾进入快照的真实订阅字段范围包括：

- 航线与行程：出发地、目的地、活跃机场组、去返日期、往返标志、航线类型；
- 乘客：总人数以及成人、儿童、老人、婴儿构成；
- 价格约束：目标价、最高预算、预算口径和目标价口径；
- 商务与时间：会议地点、开始/结束时间、会议重要性、当天往返、交通方式；
- 硬软约束：直飞、中转政策、行李要求、出行场景和行程性质；
- 通知与状态：订阅 ID、状态、通知目标和频率。

快照还携带 `price_calendar`、`price_points`、`same_day`、`analysis`、`notification`、机场到会场车程及双源比较等衍生数据，因此能组合还原价格、航班、时间窗口、推荐/排除理由和完整通知文案。对 20 个历史 blob 的值级模式扫描未发现邮箱、手机号或密钥字段命中，但上述行程画像仍属于可关联隐私。

处置结论：当前版本开始将根目录一次性快照取消跟踪，默认写入已忽略的 `data/snapshots/`。暂不改写 Git 全史；原因是当前没有直接身份信息或凭据证据，而 rewrite 会重写所有后续提交哈希、破坏公开链接和冻结基线，并要求现有克隆重新同步。若后续发现可直接关联个人的值或凭据，再单独审批历史改写并同步轮换相关凭据。
